...
// Keep a loose shape check to guard types, but do not enforce
// "required" here to avoid duplicating error entries. We perform
// explicit required/format/order validations below for consistent contract.
...
export function validatePlanInput(payload: any):
  ...
  // Ensure the payload is at least an object with expected fields
  ...
  if (errors.length > 0) {
    // Deduplicate by (field|code) pair to keep contract stable
